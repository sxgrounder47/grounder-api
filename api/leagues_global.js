module.exports = async (req, res) => {
  ...
  try {
    const [fd, tsdb] = await Promise.allSettled([fetchFDCompetitions(), fetchTSDBLeagues()]);
...
async function fetchFDCompetitions() {
  ...
  return (data.competitions || []).map((c) => ({
    ...
    type: c.type || null, // LEAGUE / CUP
    code: c.code || null
  }));
}

async function fetchTSDBLeagues() {
  const key = process.env.THESPORTSDB_API_KEY || "1";
  const r = await fetch(`https://www.thesportsdb.com/api/v1/json/${key}/all_leagues.php`);
  if (!r.ok) return [];
  const data = await r.json();
  const leagues = data.leagues || [];
  // on garde uniquement Soccer
  return leagues
    ...
    .map((l) => ({
      ...
      type: l.strLeagueAlternate ? "LEAGUE" : "LEAGUE",
      code: null
    }));
}
